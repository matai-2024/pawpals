...
const HomeHeading: React.FC<Props> = ({
  ...
  <>
    <div className={`flex flex-col md:flex-row max-w-5xl w-full gap-10`}>
      <img
        className={`object-cover md:w-1/2 h-60 w-full rounded-2xl ${direction === ImgDirection.Left ? 'flex' : 'flex md:hidden'}`}
        src={img}
        alt={alt}
      />
      <div className="md:w-1/2 md:h-[350px] w-full flex-col justify-start items-start pl-4 gap-6">
        ...
        <div className={`self-stretch justify-start items-start`}>
          <p className="text-start break-keep line-clamp-[12] pt-6 grow shrink basis-0 text-[#1e1e1e] text-base font-normal leading-snug">
            {body}
          </p>
        </div>
      </div>
